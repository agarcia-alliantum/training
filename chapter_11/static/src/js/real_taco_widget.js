...
odoo.define('tacos.section_and_note_and_tacos_backend', function (require) {
    ...
    var SectionAndNoteListRenderer = ListRenderer.extend({
        ...
        _onAddRecord: function (ev) {
            ...
            this.unselectRow().then(function () {
                ...
                // HACK
                if (context && pyUtils.py_eval(context).open_taco_configurator){
                    self._rpc({
                        model: 'ir.model.data',
                        method: 'xmlid_to_res_id',
                        kwargs: {xmlid: 'nombre wizard'},
                    }).then(function (res_id) {
                        self.do_action({
                            name: _t('Titulo wizard'),
                            type: 'ir.actions.act_window',
                            res_model: 'modelo.del.wizard',
                            views: [[res_id, 'form']],
                            target: 'new',
                            context: {
                ...
                } else if (context && pyUtils.py_eval(context).open_product_configurator){
                    self._rpc({
                        model: 'ir.model.data',
                        method: 'xmlid_to_res_id',
    ...
    fieldRegistry.add('section_and_note_and_taco_one2many', SectionAndNoteFieldOne2Many);
    fieldRegistry.add('section_and_note_and_taco_text', SectionAndNoteFieldText);
